...
/** \brief Print a debugging message plus the contents of a frame.
    ...
    \arg \c l4_protocol The protocol number (e.g., 9 for ARP) for the L4 header.  Only necessary when passing an L4 point in via the frame parameter.
 */
extern void fish_debugframe(int level, const char* msg, const void *frame, int layer, int len, uint8_t l4_protocol);
...
extern struct fishnet_fwtable_functions {
   /** \ingroup FWD
     \brief Add an entry into the forwarding table.
       ...
       can be less than 0.
       \arg \c type Indicates where this route was learned.  Should be one of
       the FISH_FWD_TYPE_* constants.
